...
from view9 import Ui_MainWindow # created with QtDesigner and pyuic
...
JoySpeedRpm = [0.5,0.6,0.7,0.8,0.9,1,1.5,2,2.5,3,4,5,6,7,8,9,10,11,\
...
class MyForm(QtGui.QMainWindow,Ui_MainWindow):
	...
	def __init__(self,parent=None):
		...
		for i in range(len(JoySpeedRpm)):
			
			self.tabJoySpeed.append(JoySpeedRpm[i]*self.convRad*self.SpeedDiv)
		...
		self.pushButtonSpeedPlus.clicked.connect(self.speedPlus)
		self.pushButtonSpeedMin.clicked.connect(self.speedMinus)
	...
	def init_curve(self, plot, title, xmin=0, xmax=100,ymin=0, ymax=100,flag=True,idView=0):
		...
		if (idView==0):
			...
			self.curve2.setPen(Qt.QPen(Qt.Qt.red, 1.5))
